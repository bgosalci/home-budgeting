...
import { calculateDayTotals } from './modules/calendar.js';
  ...
  const UI = (()=>{
    ...
    const runAnalysis = ()=>{
      const EXCLUDED_INCOME = new Set(['carried forward']);
      ...
      if(opt === 'budget-spread'){
        ...
        els.analysisMonth.innerHTML = months.map(m=>`<option value="${m}">${new Date(m+'-01').toLocaleString(undefined,{month:'short',year:'numeric'})}</option>`).join('');
        ...
        for(const m of Object.values(allMonthsData)) (m.incomes||[]).forEach(i=>{ if(!isExcluded(i.name)) catSet.add(i.name); });
        ...
        const allCheckedBefore = els.analysisIncomeCats.querySelector('input[value="__all__"]')?.checked ?? true;
        ...
        const syncAll = ()=>{
          ...
          catCbs().forEach(cb=>{ cb.disabled = on; if(on) cb.checked = false; });
          runAnalysis();
        };
        allCb.onchange = syncAll;
        catCbs().forEach(cb=>{ cb.onchange = runAnalysis; });
        document.getElementById('income-cats-select-all').onclick = ()=>{
          allCb.checked = false;
          catCbs().forEach(cb=>{ cb.disabled = false; cb.checked = true; });
          runAnalysis();
        };
        document.getElementById('income-cats-clear').onclick = ()=>{
          allCb.checked = true;
          catCbs().forEach(cb=>{ cb.disabled = true; cb.checked = false; });
          runAnalysis();
        };
      ...
      if(opt === 'monthly-spend'){
        ...
        const yearSel = els.analysisYear.value;
        const months = monthsAll.filter(m=>!yearSel || m.startsWith(yearSel));
        const isAll = els.analysisIncomeCats.querySelector('input[value="__all__"]')?.checked ?? true;
        ...
        const data = months.map(mk=>{
          const incomes = Store.getMonth(mk)?.incomes || [];
        ...
        if(level === 'category'){
          ...
          planned = labels.map(l=>cats[l]?.budget || 0);
          actual = labels.map(l=>totals.actualPerCat[l] || 0);
        }else{
          labels = Object.keys(totals.groups).sort();
          planned = labels.map(l=>totals.groups[l]?.budget || 0);
          actual = labels.map(l=>totals.groups[l]?.actual || 0);
        }
        ...
        const percentPlugin = {
          ...
          afterDatasetsDraw(chart){
            ...
            chart.getDatasetMeta(0).data.forEach((arc,i)=>{
              ...
              ctx.save(); ctx.fillStyle='#fff'; ctx.font='14px system-ui'; ctx.textAlign='center'; ctx.textBaseline='middle';
              ctx.fillText(`${val.toFixed(1)}%`, pos.x, pos.y); ctx.restore();
            });
          }
        };
        ...
        if(style === 'pie'){
          ...
          analysisChart = new Chart(els.analysisChart.getContext('2d'), {
            type:'pie', data:{labels,datasets:[{label:'Planned %', data:plannedPct, backgroundColor:colors}]}, options:pieOpts, plugins:[percentPlugin]
          });
          analysisChartActual = new Chart(els.analysisChartActual.getContext('2d'), {
            type:'pie', data:{labels,datasets:[{label:'Actual %', data:actualPct, backgroundColor:colors}]}, options:pieOpts, plugins:[percentPlugin]
          });
        }else{
