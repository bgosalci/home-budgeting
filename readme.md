...
### Notes
The **Notes** tab opens a pop‑up dialog listing all saved notes. Each entry displays its description, note text and the time it was added, with edit and delete icon buttons for changes. Notes are stored in your browser's local storage so they are available across all months. Use the multi‑line textarea to enter longer notes.
